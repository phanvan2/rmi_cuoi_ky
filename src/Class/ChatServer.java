...
//		groups.add(new Group(idGroup, ch));
	...
	public void sendToAll(String newMessage, String idchat, String idSender) throws RemoteException{	
		...
		try {
			...
			String sql_user = "SELECT id, username FROM `user` WHERE id LIKE '"+ 4 +  "'" ; 
			User user = obj.findOneDataUser(sql_user); 
			if( user == null) {
			...
			String sql =  "INSERT INTO public_chat (idUser, nameUser,content) VALUES ('"+ user.getId() +"','"+ user.getName() +"','" + newMessage + "')"; 
